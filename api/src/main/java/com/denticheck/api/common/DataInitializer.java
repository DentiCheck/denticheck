    ...
    public void run(String... args) throws Exception {
        if (hospitalRepository.count() == 0) {
            ...
            hospitalRepository.saveAll(Arrays.asList(
                    HospitalEntity.builder().name("서울중앙치과의원").address("서울특별시 중구 세종대로 110").phone("02-123-4567")
                            .latitude(37.5665).longitude(126.9780).build(),
                    HospitalEntity.builder().name("강남바른치과").address("서울특별시 강남구 테헤란로 123").phone("02-555-7777")
                            .latitude(37.5665).longitude(126.9780).build(),
                    HospitalEntity.builder().name("부산해운대치과").address("부산광역시 해운대구 우동 456").phone("051-789-0000")
                            .latitude(35.1587).longitude(129.1604).build(),
                    HospitalEntity.builder().name("대구시티치과").address("대구광역시 중구 공평로 789").phone("053-111-2222")
        ...
        if (partnerProductRepository.count() == 0) {
            ...
            partnerProductRepository.saveAll(Arrays.asList(
                    PartnerProduct.builder().category("칫솔류").name("전동칫솔 9000 Pro").price(120000).manufacturer("오랄-C")
                            .imageUrl("https://dummyimage.com/300x300/e2e8f0/64748b&text=Oral-C").build(),
                    PartnerProduct.builder().category("치약 및 세정제").name("불소 1450ppm 고함량 치약").price(8500)
                            .manufacturer("메디케어")
                            .imageUrl("https://dummyimage.com/300x300/e2e8f0/64748b&text=Toothpaste").build(),
                    PartnerProduct.builder().category("구강용품").name("치간칫솔 SSS 믹스").price(4500).manufacturer("자이덴")
                            .imageUrl("https://dummyimage.com/300x300/e2e8f0/64748b&text=Interdental").build()));
        }
        ...
        if (insuranceProductRepository.count() == 0) {
            ...
            insuranceProductRepository.saveAll(Arrays.asList(
                    InsuranceProduct.builder().category("치아보험").name("든든한 치아보장보험").price(25000).company("A+화재").build(),
                    InsuranceProduct.builder().category("종합보험").name("우리아이 첫치아보험").price(18000).company("C-보험")
                            .build()));
        }
